    ...
    override fun onFillRequest(
        ...
        try {
            ...
            if (webDomain?.isNotEmpty() == true) {
                storageManager.getPasswords().forEach { password ->
                    if (password.label.contains(webDomain!!, true) ||
                ...
                if (!(applicationName.isNotEmpty() &&
                    ...
                    webDomain != null)) {
                    storageManager.getPasswords().forEach { password ->
                        if (password.label.contains(applicationName, true) ||
                            password.url.contains(
                                applicationName,
                                true,
                            )) {
            ...
            if (usernameIds.isNotEmpty() && passwordIds.isNotEmpty()) {
                usernameIds.forEach { usernameId ->
                    dataSets.forEach { dataSet ->
                        val unknownUsernamePresentation = RemoteViews(packageName, R.layout.simple_list_item_1)
                        unknownUsernamePresentation.setTextViewText(
                            R.id.text1,
                            "\uD83D\uDC64❔" + dataSet.label + " - " + dataSet.username,
                        )
                        val unknownPasswordPresentation = RemoteViews(packageName, R.layout.simple_list_item_1)
            ...
            //
            // 					val usernamePresentation = RemoteViews(packageName, R.layout.simple_list_item_1)
            // 					usernamePresentation.setTextViewText(R.id.text1, "\uD83D\uDC64 " + dataset.label + " - " +
            // dataset.username)
            // 					val focusedUsernamePresentation = RemoteViews(packageName, R.layout.simple_list_item_1)
            // 					focusedUsernamePresentation.setTextViewText(R.id.text1, "\uD83D\uDC64❔" + dataset.label + " - " +
            // dataset.username)
            // 					val passwordPresentation = RemoteViews(packageName, R.layout.simple_list_item_1)
            // 					passwordPresentation.setTextViewText(R.id.text1, "\uD83D\uDD11 " + dataset.label + " - " +
            // dataset.username)
            //
            //
            ...
            // 						} else  {
            // 							focusedIds.forEach { dataSet.setValue(it, AutofillValue.forText(dataset.username),
            // focusedUsernamePresentation) }
            // 						}
            // 					}
            // 					if (passwordIds.isNotEmpty()) {
            // 						passwordIds.forEach { dataSet.setValue(it, AutofillValue.forText(dataset.password),
            // passwordPresentation) }
            // 					}
            //
            //
            ...
            // //							dataSetFocusedUsername.setValue(focusedId, AutofillValue.forText(dataset.username),
            // usernamePresentation)
            // //							fillResponseBuilder.addDataset(dataSetFocusedUsername.build())
            // //							dataSetFocusedPassword.setValue(focusedId, AutofillValue.forText(dataset.password),
            // passwordPresentation)
    ...
    private fun traverseNode(
        ...
        try {
            if (viewNode != null) {
                ...
                val children: List<ViewNode> = viewNode.run { (0 until childCount).map { getChildAt(it) } }

                children.forEach { childNode: ViewNode -> traverseNode(childNode, viewnodeCeptionDepth + 1) }
            }
        } catch (e: Exception) {
            e.printStackTrace()
        }
    ...
    private fun printDetails(
        ...
        try {
            if (viewNode != null) {
                ...
                GF.println(
                    "autofillHints: " +
                        (viewNode.autofillHints?.toList()?.forEach { GF.println("autofillhints: $it") }),
                )
